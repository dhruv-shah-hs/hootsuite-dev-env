...
To optionally prompt whether to show/run the local service command (TTY), run
`python3 .cursor/tools/start-service.py` after you have saved context (see `.cursor/agents/start-service.mdc`).
...
This script only lists/fetches from Jira and prints task JSON. Persist with `save-task-context.py`
(stdin). Service start prompts live in `start-service.py` (see `.cursor/agents/start-service.mdc`).
"""
